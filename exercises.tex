...
\[s(k)=\sum_{n=0}^k x_n\quad s(\infty)=\sum_{n=0}^{\infty}x_n\qquad\text{and}\qquad t(k)=\sum_{n=0}^k\norm{x_n}\quad t(\infty)=\sum_{n=0}^{\infty}\norm{x_n}\]
are \emph{continuous} on \(\N\union\{\infty\}=\Nex\subseteq\Rex\). By continuity of the norm, the composite function \(k\mapsto\norm{s(k)}\) is also continuous on~\(\Nex\). Now \(\norm{s(k)}\le t(k)\) for all \(k\in\N\) and \(\N\)~is dense in~\(\Nex\), hence \(\norm{s(\infty)}\le t(\infty)\) by~(3.15.4), as desired. Other applications of the principle of extension are similar.
\end{rmk}
